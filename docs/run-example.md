...
- `main.js` — reads the entity catalog from WASM, builds the table, `GET`/`POST`/`PUT`s `http://localhost:8080/{entity}`
...
| `src/frontend/main.js` | nav + table from the catalog; `GET`/`POST`/`PUT /{entity}` |
| `src/frontend/index.html` | shell: nav, empty table + status |
| `src/http/main.zig` | in-memory `GET`/`POST`/`PUT /{entity}` from `system` |
